...
for i in range(len(args.symmetry_list)):
    ...
    analysis_universe.load_new (coordinates[i*original_trajectory_n_frames: (i+1)*original_trajectory_n_frames-1])

    with mda.Writer((os.path.join(dir_root, 'symmetry' + str(i) + '.xtc')), analysis_universe.atoms.n_atoms) as W:
