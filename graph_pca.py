...
for i in range(num_pca_components_to_graph):
    ...
    for j in range(num_replicates):
        ...
        t = curr_dat[start_indices[j]:start_indices[j+1]:20,0]*0.001
        y = curr_dat[start_indices[j]:start_indices[j+1]:20,1]
        if np.max(t) > t_max:
            t_max = np.max(t)
        if np.min(t) < t_min:
            t_min = np.min(t)
        ...
        plot_array[i].set_xlabel('Time (ns)',fontsize=20)
        plot_array[i].set_ylabel('PC' + str(i+1) + ' Projection (nm)',fontsize=20)
